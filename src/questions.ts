...
import { mergeQuestions, type QuestionOverrides } from "./overrides";
import type { DiffChunk, PrMeta } from "./types";

/** Ids the composition layer reads; anything else is a custom question. */
export const BUILTIN_CHUNK_IDS = new Set([
  "risk",
  "has_bug",
  "needs_tests",
  "security_sensitive",
...
/**
 * Atomic question set for one diff chunk, built-ins merged with consumer
 * overrides. Each question is a single snap judgment; composition and
 * thresholds live in review.ts, never in prompts.
 */
export function buildChunkQuestions(overrides?: QuestionOverrides) {
  return mergeQuestions(BUILTIN_CHUNK_QUESTIONS, overrides);
}
